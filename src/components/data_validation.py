...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        ...
        On Failure   :  Writes an exception log and then raise an exception
        """
        try:
            status = len(dataframe.columns) == len(self._schema_config["columns"])
            logging.info(f"Is required columns presents: [{status}]")
            return status
        except Exception as e:
            raise MyException(e, sys) from e
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                validate_error_message += "Columns are missing in training dataframe. "
            else:
                logging.info(f"All required columns present in training dataframe: {status}")
                
            status  = self.validate_number_of_columns(dataframe=test_df)
            if not status:
                validate_error_message += "Columns are missing in testing dataframe"
            else:
                logging.info(f"All required columns present in testing dataframe: {status}")
                
            # Validatin column dtype for train/test df
            ...
            validation_status = len(validate_error_message) == 0
            ...
            validation_report = {
                "validation_status" : validation_status,
                "message" : validate_error_message.strip()
            }
